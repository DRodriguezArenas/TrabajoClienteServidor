...
<template>
  <div class="home">
    <h2 class="title">Welcome to Gryffindor</h2>

    <div class="personajes-container">
      <PersonajeCard casa="Gryffindor" v-for="personaje in gryffindor" :key="personaje.id" class="personaje-card" :personaje= "personaje" :fav= "favoritos">
      </PersonajeCard>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getGryffindor() {
      this.gryffindor = personajesData.filter(personaje => personaje.casa === 'Gryffindor').map(personaje => {
        return {
          ...personaje,
