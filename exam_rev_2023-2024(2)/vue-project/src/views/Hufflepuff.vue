...
<template>
  <div class="home">
    <h2 class="title">Welcome to Hufflepuff</h2>

    <div class="personajes-container">
      <PersonajeCard casa="Hufflepuff" v-for="personaje in hufflepuff" :key="personaje.id" class="personaje-card" :personaje= "personaje" :fav= "favoritos">
      </PersonajeCard>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getGryffindor() {
      this.hufflepuff = personajesData.filter(personaje => personaje.casa === 'Hufflepuff').map(personaje => {
        return {
          ...personaje,
