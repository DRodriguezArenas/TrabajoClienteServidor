...
<template>
  <div class="home">
    <h2 class="title">Welcome to Slytherin</h2>

    <div class="personajes-container">
      <PersonajeCard casa="Slytherin" v-for="personaje in slytherin" :key="personaje.id" class="personaje-card" :personaje= "personaje" :fav= "favoritos">
      </PersonajeCard>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getGryffindor() {
      this.slytherin = personajesData.filter(personaje => personaje.casa === 'Slytherin').map(personaje => {
        return {
          ...personaje,
